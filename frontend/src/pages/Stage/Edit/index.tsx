...
const EditStage = () => {
    ...
    const findStage = async (stage_id: number) => {
        try {

            const response = await api.stages.show(stage_id ?? 0);

            const stage: StageType = response?.data;
            ...
            setValue('start_at', dayjs(formatToBrazil(stage.attributes.start_at)))
            setValue('end_at', dayjs(formatToBrazil(stage.attributes.end_at)))
            setValue('how', stage.attributes.how, {shouldDirty: true, shouldTouch: true})
            setValue('what', stage.attributes.what, {shouldDirty: true, shouldTouch: true})
    ...
    return (
        <Main>
            <Header>
                <h1 className="flex flex-row items-center gap-3">Editando {stage?.attributes.name ?? <CircularProgress color="inherit" size="2rem" />}</h1>
            </Header>
