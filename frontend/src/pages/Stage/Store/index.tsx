...
const StoreStage = () => {
    ...
    const form = useForm<StoreStageFormValues>({
        defaultValues: {
            name: '',
            area: '',
            what: '',
    ...
    const onSubmit = async (data: StageAttributes) => {
        try {
            const response = await api.stages.store(company.id, currentAction.id, data);

            if(response.success === false) {
                throw new Error(response.message)
            }

            toast('Nova etapa criada com sucesso', {
                type: 'success',
                isLoading: false,
                autoClose: 3000,
                closeButton: true,
    ...
    return (
        <Main>
            <Header description="Complete as informações abaixo para criar uma nova etapa">
